...
Verification is automatic for the exact current PR head. It requires the Issue to be open with current authorization, one active durable Issue ↔ PR link, an open non-draft default-base PR that is not behind `main`, a coherent non-escalated lifecycle, the **newest unfiltered** authoritative CI run for that PR/SHA to be completed successfully with all nine required jobs green, and an independent Codex `PASS` bound to that run ID and numeric run attempt. Any queued or in-progress matching run blocks selection until all attempts settle; a newer failed or cancelled attempt shadows older green runs.

The authoritative review format is `agent-codex-review:v3`, bound to the exact repository, Issue, PR, head SHA, authorization specification hash, CI run ID, CI run attempt, and `PASS`/`BLOCK` result. Exactly one trusted marker may exist for that complete binding: duplicate PASS markers and PASS/BLOCK conflicts fail closed. Legacy v2 and SHA-only review markers are audit history only and never satisfy the current strict consumer contract. The sole authoritative CI identity is the immutable workflow ID `331418792` and path `.github/workflows/ci.yml` pinned in trusted default-branch configuration. Helpers default omitted binding fields to this configured identity; explicit IDs or paths must match it and cannot select another authority. Reviewer, verifier, gate, and merge additionally fetch current GitHub workflow metadata and require its ID, path, and active state to agree; a candidate-controlled workflow with the display name `CI` is never authoritative, and ID/path drift fails closed. Settled exact-head runs are ordered by `run_started_at`, never by completion (`updated_at`) order. The timestamps must be present and ordered as `created_at <= run_started_at <= updated_at`; a rerun must have a start later than its creation. Missing or malformed timestamps, duplicate run IDs, and tied latest start times fail closed. An older overlapping run that finishes last cannot hide a later-started failure; a rerun of an older run can supersede a later-ID run only with a demonstrably later attempt start.

The reviewer prepares one exact CI run ID and attempt, passes that binding into its prompt and output schema, and checks the returned binding when recording the result. If the selected attempt changes during the model review, the old result cannot be rebound to the replacement run. Before each PR label, Issue label, or verification-marker write, the verifier reloads CI jobs and the selected attempt, then authorization, linkage, lifecycle (including escalation), PR/Issue state, current-main ancestry, and the independent review. Verification label changes target only the addition of `agent:verified` and removal of `agent:pr`, with another full check between them; concurrent escalation and unrelated labels cannot be overwritten by a stale full-label replacement. The original Issue and specification remain pinned across all writes.

A successful verifier records `agent-verified:v2` evidence with the same CI run ID **and attempt** and invokes `agent-verified-gate`, which revalidates both evidence records and all mutable authority before publishing the trusted autonomous-merge status. Before publishing that status, the gate also records exactly one trusted `agent-verified-gate-evidence:v1` marker bound to the repository, Issue, PR, head, specification, authoritative CI run ID, run attempt, and PASS result. Auto-merge requires both the unchanged internal gate status context and that unique exact-attempt gate record; stale, malformed, duplicate, or conflicting records fail closed. The merge controller independently selects the newest unfiltered exact-head CI run and performs another full TOCTOU re-evaluation before merging with the expected exact head SHA. A same-SHA rerun therefore requires fresh review, verification, and gate evidence. If `main` moves, the branch is reconciled without force-push and all bound evidence is regenerated.

Reusable gate executions share a concurrency group keyed by repository, PR, head SHA, and specification hash, with `cancel-in-progress: false`. After being admitted, each execution re-fetches current authority and exact CI/review/verification evidence, including its final TOCTOU check. It writes a gate marker only when the current exact binding is missing; a later serialized execution observes the existing marker and does not duplicate it. Ambiguous or conflicting records continue to fail closed.
...
`Protect main` remains the independent backstop. Following the maintainer decision on 2026-09-21 it retains eight strict CI checks, CodeQL, PR/deletion/non-fast-forward protections and no bypass actors. The agent gate is required by the autonomous merge controller, not by branch protection for manual merges. The historical `agent-ruleset-sync.yml` entrypoint performs a read-only audit and never adds the gate or changes native review requirements.
...
The deterministic formatter never checks out the PR. A trusted read-only GitHub step retrieves only authorized regular Python blobs from the exact commit tree. It rejects truncated trees, symlinks and size overruns. The isolated formatter verifies blob hashes and materializes a fresh data-only Git baseline, excluding PR configuration, hooks and attributes. The existing independent validator and publisher still apply the patch to the actual source SHA.

Formatting scope includes surviving `.py` and `.pyi` files only; deleted paths are excluded before blob retrieval. Changes involving notebooks retain the existing route because Python AST equality does not validate notebook structure.

### Reuse of a sealed result during publication failures

Builder and CI fixer retain the same validated patch and exact locally sealed commit while retrying a failed Git push. A trusted helper makes at most three attempts, with two-second pauses, and rechecks the direct parent and local HEAD plus current Issue authorization before each attempt. The fixer also rechecks the authorized specification hash, default-branch base, PR identity, same-repository branch, lifecycle and two-sided linkage. A changed remote tip is rejected; force pushes are never used. If the server accepted the commit but its response was lost, read-back recognizes the exact result and avoids another push.

Builder also checks for an existing exact-head PR before each of at most three creation attempts, recovering a lost creation response without creating another candidate. Retries run only in publication jobs, which have no OpenAI key. They neither rerun the model nor modify the sealed patch. Existing checksum, path, mode, validation and sealing checks remain mandatory before publication.

This is bounded recovery within the existing publication job, not a cross-run model cache. A lost runner, expired artifact, changed scope/base, exhausted retries, or later linkage/review failure still stops under the existing finalizer. Never restart the entire generation workflow merely to retry transport. On GitHub, rerunning failed jobs preserves successful upstream jobs; a full rerun is a separate operation and is not covered by this optimization.

API usage remains limited to five explicit model steps: authorized Builder generation (manual dispatch), eligible non-format CI repairs (failed CI or manual dispatch), independent review after eligible green CI, authorized control-plane remediation review, and the fixed one-time Issue 132 bootstrap review (manual dispatch). None has a scheduled timer. Classifiers, tests, data ingestion, paper trading and publication retries do not acquire an OpenAI API credential in this repository. Native GitHub Codex review integration is separate from these workflow secrets and is not measured by this audit.
