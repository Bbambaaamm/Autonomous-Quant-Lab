...
The authoritative review format is `agent-codex-review:v3`, bound to the exact repository, Issue, PR, head SHA, authorization specification hash, CI run ID, CI run attempt, and `PASS`/`BLOCK` result. Exactly one trusted marker may exist for that complete binding: duplicate PASS markers and PASS/BLOCK conflicts fail closed. Legacy v2 and SHA-only review markers are audit history only and never satisfy the current strict consumer contract. A successful verifier records `agent-verified:v2` evidence with the same CI run ID **and attempt** and invokes `agent-verified-gate`, which revalidates both evidence records and all mutable authority before publishing the trusted required status. The merge controller independently selects the newest unfiltered exact-head CI run and performs another full TOCTOU re-evaluation before merging with the expected exact head SHA. A same-SHA rerun therefore requires fresh review and verification evidence. If `main` moves, the branch is reconciled without force-push and all bound evidence is regenerated.
