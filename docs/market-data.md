...
Každá ekonomická verze corporate action se ukládá do immutable `corporate_action_revisions`; současná `corporate_actions` tabulka je pouze current projection pro existující paper-accounting cestu. SSE evidence se pro každý provider action vyhodnocuje podle `(received_at, event_id)`, nikdy podle provider `at`. Consecutive stejný aktivní payload zachová první `received_at`; změna payloadu, DELETE nebo návrat payloadu po intervenujícím stavu vytvoří novou kauzální incarnation. Identita revize proto zahrnuje provider, provider action ID, payload hash a `known_at`. Snapshot vybírá poslední revizi známou k `as_of`. DELETE event vytváří immutable `corporate_action_cancellations` tombstone a current projection přestane být účinná; historický snapshot před DELETE zůstává reprodukovatelný. PostgreSQL odmítá `UPDATE`/`DELETE` nad event, audit, revision a cancellation evidence pomocí databázových triggerů.

Downgrade migrace obnoví starší užší UNIQUE invariant jen tehdy, pokud data neobsahují více kauzálních incarnací stejného payloadu. Pokud je obsahují, immutable historii nemaže ani neslučuje a při downgrade k `base` ponechá na přechodném revision stole novější bezpečnější constraint, než jej následující původní migrace odstraní spolu s tabulkou.
