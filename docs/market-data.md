...
Produkční historical/readiness lookup nepoužívá globální provider event historii. REST inventory nejprve určí provider action IDs v požadovaném ekonomickém intervalu; PostgreSQL k nim doplní pouze DELETE/tombstone kandidáty pro stejný symbol a scope přes immutable `corporate_action_event_symbols` sidecar. Teprve historie těchto relevantních IDs se streamuje v kauzálním pořadí `(occurred_at, event_id)`. Sidecar je odvozený index nad immutable event envelope, je backfillovaný databázově z `symbols_json` a na PostgreSQL má stejnou UPDATE/DELETE ochranu jako ostatní evidence. Produkční provider factory wireuje výhradně tento scoped loader; full-history `corporate_action_events()` zůstává pouze legacy/audit accessor.
...
Každá ekonomická verze corporate action se ukládá do raw immutable `corporate_action_revisions`; současná `corporate_actions` tabulka je pouze mutable current projection pro existující paper-accounting cestu. Legacy duplicita prokazatelně náležející stejné nepřerušené SSE incarnation zůstává v raw tabulce, ale immutable `corporate_action_revision_canonicalizations` ji auditovatelně odkazuje na canonical revizi. Produkční PIT consumer při konstrukci nového dataset snapshotu musí používat výhradně accessor `canonical_corporate_action_revisions()`, který superseded evidence vyloučí; přímý dotaz na raw tabulku není canonical historie ani vstup ekonomických efektů.

Replay již existujícího immutable snapshotu má odlišnou autoritu: po ověření schema a content hash musí každá manifestovaná corporate action přesně odpovídat raw immutable revizi ve všech polích (`action_id`, instrument, druh, effective/known čas, hodnota a nový symbol). Takovým důkazem smí být i revize později označená canonicalization sidecarem jako superseded; sidecar mění canonical historii a mutable current projection, nikoli existenci historické evidence. Replay proto snapshot nepřepisuje na dnešní canonical revizi ani jej neporovnává s `corporate_actions`.

Snapshot i jeho raw revision evidence musí náležet stejné persistentní provider lineage. Legacy current-projection řádek bez raw revize proto nesmí být fallbackem pro nový VALID snapshot: konstrukce selže uzavřeně a vyžádá si nejprve důvěryhodnou immutable revision evidence. Historický snapshot bez takové evidence se při replay rovněž nepřijme.

SSE evidence se pro každý provider action vyhodnocuje podle `(received_at, event_id)`, nikdy podle provider `at`. Evidence používá transportní identitu `ProviderMetadata.name` (například `alpaca`), zatímco revisions a canonicalization sidecar používají lineage identitu `ProviderMetadata.persistent_name` (například `alpaca:iex`); canonicalizer tyto identity nesmí zaměňovat. Consecutive stejný aktivní payload zachová první `received_at`; změna payloadu, DELETE nebo návrat payloadu po intervenujícím stavu vytvoří novou kauzální incarnation. Canonicalizace je povolena jen při shodě revision provideru, provider action ID, payload hashe a celého ekonomického obsahu a při existenci revize přesně v prvním lokálním receipt čase. Nejednoznačný obsah selže uzavřeně. Current projection lze posunout na tento dřívější čas pouze při úplné ekonomické shodě; jiný payload ani ekonomický fakt se nevrací zpět. Snapshot vybírá poslední canonical revizi známou k `as_of`. DELETE event vytváří immutable `corporate_action_cancellations` tombstone a current projection přestane být účinná; historický snapshot před DELETE zůstává reprodukovatelný. PostgreSQL odmítá `UPDATE`/`DELETE` nad event, audit, revision, cancellation a canonicalization evidence pomocí databázových triggerů.

Downgrade canonicalization migrace explicitně selže, pokud sidecar již obsahuje immutable auditní vazbu; auditní evidence se nesmí ztratit odstraněním celé tabulky. Následující downgrade obnoví starší užší UNIQUE invariant jen tehdy, pokud data neobsahují více kauzálních incarnací stejného payloadu. Pokud je obsahují, také selže před změnou Alembic revision markeru: legitimní immutable historii nesmí mazat ani slučovat jen proto, aby ji vměstnala do staršího nereprezentovatelného schématu.
...
`AlpacaCorporateActionStream` používá `Last-Event-Id` pro reconnect/replay. Replay je inkluzivní, proto každý doručený event prochází idempotentním persistentním sinkem: identický již uložený event vrátí duplicate stav a cursor neposune, nový immutable receipt se uloží a cursor se posune atomicky ve stejné DB transakci. Listener kvůli deduplikaci nedrží neomezený `seen` set; memory complexity replay stavu je konstantní. Reconnect je bounded; transientní/provider chyby nevedou k nekonečnému retry. Standardní CI používá pouze fixture transport, nikoli externí síť.

SSE evidence se v provozu ingestuje samostatným procesem `quantlab-alpaca-events` (v developmentu lze ekvivalentně spustit `python -m quantlab.alpaca_event_worker`). Produkční Compose jej spouští jako `alpaca-events` pouze s DB a market-data egress sítí. Proces používá standardní aplikací validovanou konfiguraci a pro `MARKET_DATA_PROVIDER=alpaca` vyžaduje credentials `ALPACA_KEY_ID`/`ALPACA_SECRET_KEY`. Pro jiného providera skončí úspěšně; Compose politika `on-failure` jej proto ve Stooq režimu nerestartuje. V Alpaca režimu čte O(1) mutable transport cursor z `corporate_action_event_cursors`; legacy fallback používá indexed `ORDER BY occurred_at DESC, event_id DESC LIMIT 1` a nikdy nematerializuje celou historii. Cursor table není evidence a smí se měnit; `corporate_action_events` a audit sidecar zůstávají immutable. Proces periodicky loguje current/peak RSS, CPU time, received/written/duplicate counters a idle time, takže staging může přímo porovnat nový steady-state s původním ~1.48 GiB baseline. I čisté vyčerpání bounded SSE reconnectů skončí chybou, aby `on-failure` worker obnovil. Event-worker je oddělen od paper-trading workeru a nevytváří žádnou broker/order cestu. Produkční paper worker získává market-data provider přes společný credential-gated provider factory; live trading path tím nevzniká.
