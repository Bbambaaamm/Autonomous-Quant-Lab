...
test("new SHA cannot reuse old verification or gate evidence", () => {
  const marker = a.verificationMarker({ repo, issueNumber: 42, prNumber: 77, headSha: "a".repeat(40), specHash: spec, ciRunId: 999, ciRunAttempt: 1 });
  const comments = [{ user: { login: "github-actions[bot]" }, body: marker }];
  assert.equal(a.exactVerificationEvidence(comments, { repo, issueNumber: 42, prNumber: 77, headSha: "a".repeat(40), specHash: spec, ciRunId: 999, ciRunAttempt: 1 }), true);
  assert.equal(a.exactVerificationEvidence(comments, { repo, issueNumber: 42, prNumber: 77, headSha: "b".repeat(40), specHash: spec, ciRunId: 999, ciRunAttempt: 1 }), false);
});
...
test("Issue #112 verifier keeps metadata on GITHUB_TOKEN and reconciliation on publish token", () => {
  ...
  const marker = a.verificationMarker({ repo, issueNumber: 112, prNumber: 114, headSha, specHash: spec, ciRunId: 1234, ciRunAttempt: 2 });
  const args = { repo, issueNumber: 112, prNumber: 114, headSha, specHash: spec, ciRunId: 1234, ciRunAttempt: 2 };
...
test("Issue #140 verification evidence is bound to exact CI run and numeric attempt", () => {
  const binding = { repo, issueNumber: 140, prNumber: 141, headSha: "c".repeat(40), specHash: spec, ciRunId: 700, ciRunAttempt: 3 };
  const marker = a.verificationMarker(binding);
  const comments = [{ user: { login: "github-actions[bot]" }, body: marker }];
  assert.equal(a.exactVerificationEvidence(comments, binding), true);
  assert.equal(a.exactVerificationEvidence(comments, { ...binding, ciRunId: 701 }), false);
  assert.equal(a.exactVerificationEvidence(comments, { ...binding, ciRunAttempt: 4 }), false);
  assert.equal(a.exactVerificationEvidence(comments, { ...binding, ciRunAttempt: undefined }), false);
  assert.equal(a.exactVerificationEvidence([{ user: { login: "github-actions[bot]" }, body: marker.replace("attempt=3", "attempt=none") }], binding), false);
  assert.throws(() => a.verificationMarker({ ...binding, ciRunAttempt: "none" }), /INVALID_CI_RUN_ATTEMPT/);
});
