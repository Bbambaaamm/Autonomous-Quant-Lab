...
test("Issue #118 control-plane recovery revalidates mutable authority, ruleset, CI and ancestry before writes", () => {
  ...
  assert.match(recover, /required\.filter\(x=>x\.context===a\.GATE_CONTEXT&&x\.integration_id===15368\)\.length===1/);
  assert.doesNotMatch(recover, /expectedChecks/);
...
test("Issue #118 maintenance gate and merge preserve the exact trusted Protect main checks", () => {
  ...
  for (const section of [gate, merge]) {
    ...
    assert.doesNotMatch(section, /expectedChecks/);
...
test("Issue #118 maintenance carries and freshly revalidates requester authority through gate and merge writes", () => {
  ...
  const patStep = merge.slice(0, merge.indexOf("- name: Record completed merge"));
  const auditStep = merge.slice(merge.indexOf("- name: Record completed merge"));
  assert.doesNotMatch(patStep, /github\.rest\.issues\.createComment/);
  assert.doesNotMatch(auditStep, /AGENT_PUBLISH_TOKEN|maintainerOk|getCollaboratorPermissionLevel/);
  assert.match(auditStep, /pr\.state!==\s*'closed'/);
  assert.match(auditStep, /!pr\.merged/);
  assert.match(auditStep, /pr\.head\.sha!==headSha/);
  assert.match(auditStep, /pr\.merge_commit_sha!==mergeSha/);
  assert.match(auditStep, /p\.parseAgentIssue\(pr\.body\)!==issueNumber/);
  assert.match(auditStep, /MERGE: '\$\{\{ steps\.merge\.outputs\.merge_sha \}\}'/);
