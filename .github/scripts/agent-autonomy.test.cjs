...
test("newest CI failure defeats older success",()=>{const p=require("./agent-pipeline.cjs"),sha="d".repeat(40),b={workflow_id:4242,path:".github/workflows/ci.yml",run_attempt:1,updated_at:"2026-01-01T00:00:00Z",name:"CI",event:"pull_request",status:"completed",head_sha:sha,pull_requests:[{number:88}]};assert.equal(p.newestAuthoritativeCiRun([{...b,id:10,conclusion:"success"},{...b,id:11,conclusion:"failure"}],{workflowId:4242,workflowPath:".github/workflows/ci.yml",headSha:sha,prNumber:88}).conclusion,"failure");});
