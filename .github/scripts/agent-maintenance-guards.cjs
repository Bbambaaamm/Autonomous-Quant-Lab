...
function rulesetDecision(ruleset, expected) {
  ...
  if (!["api", "quality", "security", "unit-research", "integration-postgres", "frontend", "container-build", "production-smoke"].every(context => required.some(check => check.context === context && check.integration_id === 15368))) {
    return fail("RULESET_CI_EXPECTATION_MISSING");
  }
