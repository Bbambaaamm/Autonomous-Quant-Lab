...
const expected = {
  ...
  requiredChecks: ["api", "quality", "security", "unit-research", "integration-postgres", "frontend", "container-build", "production-smoke"]
...
const mutations = [
  ...
  ["duplicate required check", s => {s.rulesetAudit.ruleset.rules[3].parameters.required_status_checks.push(clone(expected.requiredChecks.at(-1)));}],
