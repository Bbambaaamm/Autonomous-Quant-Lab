...
async function executeIssue128FinalWrite(change={}){
  ...
  const reviewer={id:90,workflow_id:900,path:".github/workflows/agent-codex-review.yml",head_sha:base,run_attempt:4};let drift=false,newerCiStarted=false;
  ...
  const github={rest:{actions,repos:{get:async()=>({data:{full_name:"o/r",default_branch:"main"}}),getBranch:async()=>({data:{commit:{sha:change.mainDrift||base}}})},pulls:{get:async()=>({data:{state:"open",draft:false,base:{ref:change.baseDrift?"other":"main",sha:base},head:{sha:drift||change.headDrift?"c".repeat(40):head,repo:{full_name:"o/r"}},body:"Agent-Issue: #128",labels:["agent:pr"]}})},issues:{listComments(){},get:async()=>({data:{state:"open",title:"t",body:"b",labels:["type:implementation","agent:pr"]}}),createComment:async x=>writes.push(x)}},paginate:async(route)=>{if(route===actions.listJobsForWorkflowRun)return [{name:"required",status:"completed",conclusion:"success"}];if(route===actions.listWorkflowRunsForRepo)return change.newerCi||newerCiStarted?[{...ci,id:11,run_attempt:2,status:"in_progress",conclusion:null},ci]:[ci];if(change.newerCiDuringCommentPagination)newerCiStarted=true;if(change.driftDuringPagination)drift=true;return [];}};
...
test("Issue #128 final write rejects a newer exact-head CI run started during comment pagination",async()=>{
  ...
  assert.deepEqual(result.failures,["NO_WRITE: newest CI changed at final review record boundary"]);
});
