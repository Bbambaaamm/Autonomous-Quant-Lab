...
s = p.read_text()
old = '''      - name: npm audit with bounded network retry
...
'''
new = '''      - name: Production npm audit with bounded network retry
        working-directory: frontend
        env:
          npm_config_fetch_timeout: "60000"
          npm_config_fetch_retries: "1"
...
'''
if s.count(old) != 1:
    raise SystemExit(f'expected one npm audit block, got {s.count(old)}')
p.write_text(s.replace(old, new, 1))
for path in ['.github/scripts/pr108-security-finalize.py', '.github/workflows/pr108-security-finalize.yml']:
    q = Path(path)
