...
class OperatorReadModel:
    ...
    def _latest_monitoring(self, session: Session) -> PaperMonitoringRunRecord | None:
        return session.scalar(
            ...
            .where(PaperMonitoringRunRecord.paper_account_id == self.account_id)
    ...
    def _latest_snapshot(
        ...
        query = select(PaperPerformanceSnapshotRecord).where(
            PaperPerformanceSnapshotRecord.paper_account_id == self.account_id
        )
        if monitoring_id:
    ...
    def overview(self, now: datetime) -> dict[str, Any]:
        ...
        with self._session_factory() as session:
            account = session.get(PaperAccountRecord, self.account_id)
            ...
            reconciliation = session.scalar(
                ...
                .where(ReconciliationRecord.account_id == self.account_id)
                .order_by(ReconciliationRecord.timestamp.desc())
                .limit(1)
            )
            cycle = session.scalar(
                select(TradingCycleRecord)
                .where(TradingCycleRecord.account_id == self.account_id)
                .order_by(TradingCycleRecord.started_at.desc())
                .limit(1)
            )
            ...
            next_job = session.scalar(
                ...
                .where(
                    ...
                    ScheduledJob.job_type == "RUN_PAPER_CYCLE",
                    ScheduledJob.account_id == self.account_id,
                )
            ...
            return {
                ...
                "open_order_count": session.scalar(
                    ...
                    .where(
                        PaperOrderRecord.account_id == self.account_id,
    ...
    def performance(
        ...
        with self._session_factory() as session:
            ...
            if monitoring and monitoring.paper_account_id != self.account_id:
                monitoring = None
            ...
            return {
                ...
                "positions": [
                    ...
                    for x in session.scalars(
                        ...
                        .where(PositionRecord.account_id == self.account_id)
                ...
                "orders": [
                    ...
                    for x in session.scalars(
                        ...
                        .where(PaperOrderRecord.account_id == self.account_id)
            ...
            return {
                ...
                "decisions": [
                    ...
                    for x in session.scalars(
                        ...
                        .where(RiskDecisionRecord.account_id == self.account_id)
                ...
                "events": [
                    ...
                    for x in session.scalars(
                        ...
                        .where(RiskEventRecord.account_id == self.account_id)
    ...
    def data_health(self, now: datetime) -> dict[str, Any]:
        ...
        with self._session_factory() as session:
            ...
            monitoring = self._latest_monitoring(session)
            deployment = (
                session.get(StrategyDeploymentRecord, monitoring.deployment_id)
                if monitoring
                else None
            )
                ...
                set(
                    session.scalars(
                        select(UniverseMembershipRecord.instrument_id).where(
                            ...
                            UniverseMembershipRecord.valid_from < session_end,
                            (UniverseMembershipRecord.valid_to.is_(None))
                            | (UniverseMembershipRecord.valid_to >= session_start),
                            UniverseMembershipRecord.known_at < session_end,
                        )
                    )
                )
                if deployment
    ...
    def comparison(self, monitoring_id: str) -> dict[str, Any] | None:
        with self._session_factory() as session:
            ...
            if not run or run.paper_account_id != self.account_id:
                return None
