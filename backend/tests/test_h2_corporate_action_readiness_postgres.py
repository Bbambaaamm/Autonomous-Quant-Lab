...
def test_alpaca_event_cursor_is_atomic_idempotent_and_stream_ordered(scope) -> None:
    factory, _ = scope
    service = PersistentMarketDataService(factory)
    suffix = uuid4().hex[:16]
    ...
    def event(event_id: str) -> CorporateActionEvent:
    ...
    first = event(f"z-{suffix}")
    second = event(f"a-{suffix}")
    ...
    assert service.record_corporate_action_event(provider, first) is False
    assert service.latest_corporate_action_event_id(provider) == first.event_id

    # Same receipt time, reverse lexicographic ID: operational cursor follows stream order.
