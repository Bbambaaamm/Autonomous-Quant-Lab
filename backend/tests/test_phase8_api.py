...
def test_audit_filters_are_server_side_and_range_is_validated(tmp_path, monkeypatch):
    ...
    assert api.get("/operator/audit?start_utc=2026-02-01T00:00:00").status_code == 422
    assert (
        api.get(
            "/operator/audit?start_utc=2026-02-01T00:00:00&end_utc=2026-02-02T00:00:00Z"
        ).status_code
        == 422
    )
...
def test_data_health_ignores_unmonitored_registry_instruments(tmp_path, monkeypatch):
    api = client(tmp_path, monkeypatch)
    ...
    assert covered["relevant_instrument_count"] == 0
    assert covered["current_observation_count"] == 0
...
def test_account_specific_projections_do_not_mix_secondary_account(tmp_path, monkeypatch):
    api = client(tmp_path, monkeypatch)
    ...
    import quantlab.api as module
    from quantlab.phase4 import PositionRecord, RiskEventRecord

    module.paper_repository.seed_account("paper-secondary", Decimal("50000"))
    now = datetime.now(UTC)
    with Session(module.paper_repository.engine) as session:
        session.add(
            PositionRecord(
                account_id="paper-secondary",
                instrument_id="SECONDARY_ONLY",
                quantity=Decimal("10"),
        ...
        session.add(
            RiskEventRecord(
                id="secondary-risk-event",
                account_id="paper-secondary",
                timestamp=now,
                event_type="SECONDARY_ONLY",
                reason="nesmí uniknout do paper-main projekce",
                correlation_id="secondary-correlation",
            )
        )
        session.commit()

    paper = api.get("/operator/paper").json()
    risk = api.get("/operator/risk").json()
    assert all(row["instrument_id"] != "SECONDARY_ONLY" for row in paper["positions"])
    assert all(row["event_type"] != "SECONDARY_ONLY" for row in risk["events"])
