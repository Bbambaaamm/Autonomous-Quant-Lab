...
def test_audit_filters_are_server_side_and_range_is_validated(tmp_path, monkeypatch):
    api = client(tmp_path, monkeypatch)
    api.post("/operator/risk/halt", json={"confirmation": "HALT", "reason": "audit event"})
    result = api.get("/operator/audit?event_type=SYSTEM_HALTED&limit=1").json()
    assert result["limit"] == 1
    assert result["total"] >= 1
    assert all(item["event_type"] == "SYSTEM_HALTED" for item in result["items"])
    assert (
        api.get(
