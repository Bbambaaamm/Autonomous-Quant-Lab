...
install_operational_timers() {
    ...
    install -m 0644 "$REPO/ops/systemd/quantlab-soak-sample.service"         /etc/systemd/system/quantlab-soak-sample.service
    install -m 0644 "$REPO/ops/systemd/quantlab-soak-sample.timer"         /etc/systemd/system/quantlab-soak-sample.timer
    systemctl daemon-reload
    systemctl enable --now quantlab-db-backup.timer quantlab-soak-sample.timer >/dev/null
}
