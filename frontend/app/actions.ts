...
export async function ingestionAction(_: ActionState, form: FormData) { return result(() => mutate("/operator/market-data/ingestions", { instrument_id: value(form, "instrument_id"), start: value(form, "start"), end: value(form, "end"), reason: value(form, "reason") }), "Import byl dokončen a tabulka aktualizována."); }
export async function snapshotAction(_: ActionState, form: FormData) { return result(() => mutate("/operator/datasets", { universe_id: value(form, "universe_id"), start: value(form, "start"), end: value(form, "end"), as_of: utcValue(form, "as_of"), minimum_coverage: value(form, "minimum_coverage"), reason: value(form, "reason") }), "Neměnná verze dat byla uložena."); }
