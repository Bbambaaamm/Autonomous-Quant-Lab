...
export async function eligibilityAction(_: ActionState, form: FormData) { const id=value(form,"id"); return result(()=>mutate(`/operator/research/experiments/${id}/eligibility`,{reason:value(form,"reason")}),"Eligibility byla autoritativně vyhodnocena."); }
export async function promotionAction(_: ActionState, form: FormData) { const id=value(form,"id"); return result(()=>mutate(`/operator/research/experiments/${id}/promote`,{reason:value(form,"reason")}),"Experiment byl promován na PAPER_CANDIDATE."); }
...
export async function approvalAction(_: ActionState, form: FormData) { const id=value(form,"deployment_id"); return result(()=>mutate(`/operator/deployments/${id}/approve`,{reason:value(form,"reason")}),"Paper deployment byl schválen."); }
export async function autonomousScheduleAction(_: ActionState, form: FormData) { const id=value(form,"deployment_id"), action=value(form,"action"); return result(()=>mutate(`/operator/deployments/${id}/autonomous/${action}`,{reason:value(form,"reason")}),`Autonomous scheduling byl ${action==="enable"?"zapnut":"vypnut"}.`); }
export async function monitoringEnrollmentAction(_: ActionState, form: FormData) { return result(()=>mutate("/operator/monitoring/enrollments",{deployment_id:value(form,"deployment_id"),policy_id:value(form,"policy_id"),reason:value(form,"reason")}),"Deployment byl zařazen do monitoringu."); }
...
export async function monitoringAction(_: ActionState, form: FormData) { const id=value(form,"id"), action=value(form,"action"); return result(()=>mutate(`/operator/monitoring/${id}/${action}`,{reason:value(form,"reason")}),`Monitoring transition ${action.toUpperCase()} byla potvrzena serverem.`); }
...
export async function membershipAction(_: ActionState, form: FormData) { const id=value(form,"universe_id"); return result(()=>mutate(`/operator/universes/${id}/memberships`,{instrument_id:value(form,"instrument_id"),valid_from:utcValue(form,"valid_from"),known_at:utcValue(form,"known_at"),reason:value(form,"reason")}),"Point-in-time membership byla přidána."); }
