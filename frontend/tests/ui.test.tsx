...
import { Status, pct, JsonTable } from "../components/ui";
import { LineChart } from "../components/chart";
describe("pravdivá operator prezentace", () => { it("zobrazuje HALTED jako unsafe", () => { render(<Status value="HALTED"/>); expect(screen.getByTitle("HALTED")).toHaveClass("unsafe"); }); it("SUSPENDED je viditelně unsafe", () => { render(<Status value="SUSPENDED"/>); expect(screen.getByTitle("SUSPENDED")).toHaveClass("unsafe"); }); it("INSUFFICIENT_DATA není healthy", () => { render(<Status value="INSUFFICIENT_DATA"/>); expect(screen.getByTitle("INSUFFICIENT_DATA")).toHaveClass("watch"); }); it("null return není 0 %", () => expect(pct(null)).toBe("Neuvedeno")); it("prázdná performance řada je pravdivá", () => { render(<LineChart points={[]} field="marked_equity" label="Equity"/>); expect(screen.getByText(/nejsou dostupné/)).toBeInTheDocument(); }); it("tabulka vykreslí přesná API data", () => { render(<JsonTable rows={[{ instrument_id: "SPY", quantity: "2.50000000" }]}/>); expect(screen.getByTitle("2.50000000")).toBeInTheDocument(); }); });
describe("kritické data statusy", () => {
    it("missing session data je unsafe", () => {
        render(<Status value="MISSING_SESSION_DATA"/>);
        expect(screen.getByTitle("MISSING_SESSION_DATA")).toHaveClass("unsafe");
    });
});
describe("autonomous worker readiness", () => {
    it("nedostupný worker není zobrazen jako bezpečný", () => {
        render(<Status value="UNAVAILABLE"/>);
        expect(screen.getByTitle("UNAVAILABLE")).toHaveClass("unsafe");
    });
    it("globálně vypnutý runtime není zobrazen jako ready", () => {
        render(<Status value="DISABLED"/>);
        expect(screen.getByTitle("DISABLED")).toHaveClass("unsafe");
    });
});
