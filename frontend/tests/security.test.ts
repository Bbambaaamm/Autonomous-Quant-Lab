...
describe("paper-only hranice", () => {
    it("navigace neobsahuje live akci ani generic proxy", () => {
        ...
        expect(layout).toContain("Bez skutečných objednávek");
    });
    ...
    it("FAILED/DEAD_LETTER recovery používá pouze reasoned operator endpoint", () => {
        ...
        expect(operations).toContain("Důvod změny");
...
describe("Phase 9 security boundary", () => {
    ...
    it("dev helper vytváří obě strany role credentials a login identitu", () => {
        ...
        for (const variable of ["OPERATOR_USERNAME", "QUANTLAB_API_VIEWER_TOKEN", "QUANTLAB_API_OPERATOR_TOKEN", "QUANTLAB_API_ADMIN_TOKEN"])
