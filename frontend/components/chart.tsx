...
export function LineChart({ points, field, label, currency }: {
    points: Point[];
    ...
    return <section className="card"><h2>{label}</h2><p><strong>{format(values.at(-1)!)}{field === "marked_equity" ? ` ${currency ?? "(měna neuvedena)"}` : ""}</strong> <span className="muted">· poslední hodnota · {points.length} seancí</span></p><svg className="chart" viewBox="0 0 1000 240" role="img" aria-label={`${label}, ${points.length} seancí`}><title>{`${dateText(points[0].session_date)} – ${dateText(points.at(-1)!.session_date)}`}</title>{[40, 115, 190].map(y => <line key={y} x1="100" x2="920" y1={y} y2={y} stroke="#2b3c52"/>)}<text x="0" y={max === min ? "120" : "45"} fill="#a3b5ca" fontSize="14">{format(max)}</text>{max !== min && <text x="0" y="195" fill="#a3b5ca" fontSize="14">{format(min)}</text>}<path d={path} fill="none" stroke="#61dfbc" strokeWidth="3"/>{values.length === 1 && <circle cx="100" cy="115" r="5" fill="#61dfbc"/>}<text x="100" y="230" fill="#a3b5ca" fontSize="14">{dateText(points[0].session_date)}</text><text x="920" y="230" textAnchor="end" fill="#a3b5ca" fontSize="14">{dateText(points.at(-1)!.session_date)}</text></svg><p className="paper-note">Uzavřené obchodní seance XNYS. Přesné hodnoty jsou dostupné v detailu monitoringu.</p></section>;
}
