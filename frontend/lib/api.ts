...
export async function api<T>(path:string):Promise<T>{
  ...
  if(!response.ok){const payload=await response.json().catch(()=>null) as {detail?:unknown}|null;const detail=typeof payload?.detail==="string"?payload.detail:response.status===503?"Služba není připravena":`API chyba ${response.status}`;throw new ApiError(response.status,detail)}
  return response.json() as Promise<T>;
}
export type Overview={trading_mode:string;live_trading_enabled:boolean;api_health:string;readiness:string;autonomous_readiness:string;autonomous_workload_enabled:boolean;paper_account_id:string|null;trading_state:string|null;reconciliation_safe:boolean|null;latest_reconciliation_status:string|null;monitoring_id:string|null;monitoring_state:string|null;monitoring_verdict:string|null;paper_equity:string|null;paper_cash:string|null;cumulative_return:string|null;current_drawdown:string|null;position_count:number;open_order_count:number;last_trading_cycle:string|null;next_scheduled_paper_cycle:string|null;latest_completed_market_session:string;latest_market_data_status:string|null;latest_market_data_at:string|null;automation_enabled:boolean;enabled_job_count:number;dead_letter_count:number;healthy_worker_count:number;stale_worker_count:number;stopped_worker_count:number;latest_paper_run:Record<string,unknown>|null;server_time_utc:string;as_of:string|null};
